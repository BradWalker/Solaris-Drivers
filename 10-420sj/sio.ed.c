...
*/
static char CopyId[] = "@(#)Copyright (c) 1992 Aurora Technologies, Inc.";
static char SccsId[] = "@(#)Aurora Technologies, %W% %G% %R% [sio Version 999]";
...
sio_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	if ((iblock == NULL) ||
		((int)sio_board_ptr->idevice_cookie.idev_priority > maxprio)) {
		maxprio = (int)sio_board_ptr->idevice_cookie.idev_priority;
		iblock = &(sio_board_ptr->iblock_cookie);
	}
...
attach_failed:
        /*
         * need to add the code here to kmem free
 ...
 */
static int
sio_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
